...
// gopus 仅在 Discord 语音功能中使用，如果不需要可以移除
// 要移除 gopus 依赖，请注释掉下面的 import 和相关的语音编码/解码函数
...
func (d *DiscordRobot) sendAudioToDiscord(_ *discordgo.VoiceConnection, _ []byte) {
	// Discord 语音功能需要 gopus 库
	// 如果不需要 Discord 语音功能，可以移除 gopus 依赖并注释掉此函数
	logger.Warn("Discord voice feature requires gopus library. Install with: go get layeh.com/gopus")
	logger.Warn("To disable Discord voice, comment out this function and receiveVoice function")

	// 取消注释下面的代码以启用 Discord 语音功能（需要 gopus）
	/*
...
func (d *DiscordRobot) receiveVoice(vc *discordgo.VoiceConnection) {
	...
	// 如果不需要 Discord 语音功能，可以移除 gopus 依赖并注释掉此函数
	logger.Warn("Discord voice feature requires gopus library. Install with: go get layeh.com/gopus")
	logger.Warn("To disable Discord voice, comment out this function and sendAudioToDiscord function")
