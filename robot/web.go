...
func (web *Web) sendTextStream(messageChan *MsgChan) {
	logger.Debug("sendTextStream started, waiting for messages")
	totalContent := ""
	for msg := range messageChan.StrMessageChan {
		...
		logger.Debug("sendTextStream received message", "length", len(msg), "preview", preview)
		// 调试：输出发送的内容（特别是 thinking 标签）
		if strings.Contains(msg, "<think>") {
			contentPreview := msg
			if len(msg) > 200 {
				contentPreview = msg[:200] + "..."
			}
			logger.Debug("sending thinking content to HTTP", "content", contentPreview)
		}

		// 尝试写入响应，如果失败（client 已断开），记录日志但继续处理
		// 这样 agent 任务可以继续执行，即使 client 已断开
		if _, err := fmt.Fprintf(web.W, "%s", msg); err != nil {
			logger.WarnCtx(web.Robot.Ctx, "failed to write to response (client may have disconnected), continuing agent task", "err", err)
			// 继续收集内容，以便后续保存到数据库
		}
		totalContent += msg

		// 尝试刷新（Flush 不返回错误，但如果 client 已断开，后续写入会失败）
		web.Flusher.Flush()
	}
